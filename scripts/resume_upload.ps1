...
Write-Host "🚀 Resuming D1 bulk upload..." -ForegroundColor Yellow
Write-Host "📦 Uploading batches $startBatch-$endBatch ($totalBatches remaining batches)" -ForegroundColor Cyan
Write-Host "⏱️ Estimated time: ~$([math]::Ceiling($totalBatches * 2 / 60)) minutes" -ForegroundColor Cyan
Write-Host ""
...
for ($i = $startBatch; $i -le $endBatch; $i++) {
    ...
    if (-not (Test-Path $batchFile)) {
        Write-Host "❌ Batch file $batchFile not found" -ForegroundColor Red
        $failedBatches += $i
        continue
    }
...
Write-Host "⏱️ Total time: $([math]::Round($totalTime, 1)) minutes" -ForegroundColor Cyan
