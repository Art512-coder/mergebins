...
# We already have batches 1-419 uploaded (209,500 records)
param(
    [int]$StartBatch = 420,
    [int]$EndBatch = 917
)

Write-Host "Starting D1 bulk upload resume from batch $StartBatch..." -ForegroundColor Green
...
try {
    ...
    Write-Host "Could not verify final count - please check manually with:" -ForegroundColor Yellow
    Write-Host "wrangler d1 execute bin-search-db --command='SELECT COUNT(*) as total_records FROM bins;'" -ForegroundColor Yellow
}
